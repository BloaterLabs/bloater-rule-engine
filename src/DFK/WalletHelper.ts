...
export class WalletHelper {
  public async createWallet(path: string, provider: Provider, password: string = null) {
    if (!password) {
  ...
  public async getEncryptedWallet(path: string, provider: Provider, password: string = null) {
    if (!password) {
      password = await this.getInput('Password:', 'password');
    }
  ...
  public async getOrCreateWallet(path: string, provider: Provider, password: string = null) {
  ...
  private async getInput(query, inputName): Promise<string> {
    ...
    try {
      ...
      if (!input) {
        throw new Error(`No ${inputName} provided. Try running the application again, and provide a ${inputName}.`);
      }
