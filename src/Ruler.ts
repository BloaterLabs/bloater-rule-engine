...
export class Ruler {
    ...
    constructor(private wallet: Wallet, config: Config) {
        ...
        this.questCore = new QuestCore(addresses, provider, wallet);
        this.heroCore = new HeroCore(addresses, provider);
    }
    ...
    public setupEngine() {
        ...
        this.rulesEngine.addFact('dfk-heroes-questing', (params, almanac) => 
            almanac.factValue(params.address)
                .then((address: string) => this.getQuestingHeroes(address)));

        this.rulesEngine.addFact('dfk-heroes', (params, almanac) => 
            almanac.factValue(params.address)
                .then((address: string) => this.getHeroes(address)));
    ...
    private startQuests(heroes: Hero[], requiredStamina: number): void {
        ...
        Promise.all(heroes.map((hero: Hero) => {
